...
void squares(int size1, int x_offset, int y_offset, int size2) {
  ...
  int w = max(x_offset, x_offset+size2) ;
  //compute the max of size1 and (y_offset + size2).  Call this h
  int h = max(y_offset, y_offset+size2) ;
  ...
  for(int y=0; y< h; y++) {
	...
	printf("*");
      ...
      // if so, print a *
      else if((isInRange(x, 0, size1) && isAtBorder(y, 0,size1-1)) || (isInRange(y, 0, size1) && isAtBorder(x, 0, size1-1)))
	printf("#");
      //if not,
      // check if EITHER
